//high score stuff
let reader; //reader for high score text file
let output; //writer for high score text file
let highscore; //high score value
...
let soundp = 1;
let score = 0; //score
let justScored; //held score for use across modes
let mode = 0; //mode, for which screen the player is in, including all of the offshoots of the menu and the actual gameplay
let cooldown = 0; //cooldown for flapping
let pipeSpeed = 3; //speed the pipes move towards the player at
let pipeSpace = 100; //number of frames in between each pipe spawn
let bkgX = 0; //x value of the first background image
let bkg2X = 1000; //x value of the copy of the first background image, so it can scroll and repeat
let skyX = 0; //x value of the secondary background image
let sky2X = 1000; //x value of the copy of the secondary background image, so it can scroll and repeat
let sky3X = 0; //x value of the tertiary background image
let sky4X = -4000; //x value of the copy of the tertiary background image, so it can scroll and repeat
let sansX = 0; //x value of the background image for sans mode
let sans2X = 1000; //x value of the copy of the background image for sans mode, so it can scroll and repeat
let colourPalette = 1; //which color mode the player is in
let beginnerPipe = 0; //counter so after the first few pipes the beginner dots stop appearinglet instructionNum = 0; //counts which instruction menu the player is on
let instructionNum = 0; //counts which instruction menu the player is on

let space = false; //boolean to prevent the space key from repeating without being released
let isAlive = true; //boolean for if the palyer is alive or not
let ProMode = false; //checks if player is in pro mode with high contrast pipes
let justClicked = false; //boolean to prevent clicking from repeating without being released
let sans = false; //checks if the player is in sans mode

let text; //starting text color
let background; //starting background color
let box ; //starting box (for menus) color
let ProBox; //starting color for the promode box so it can flip when clicked
let ProText ; //starting color for the promode text so it can flip when clicked
let pipeColor; //starting color for the pipes
let dot ; //starting color for the training dots
...
{
    //sprites
    sprite = loadImage("scripts/assets/rossJamieson.png");
    spriteAlt0 = loadImage('scripts/assets/rossJamieson.png');
    spriteAlt1 = loadImage('scripts/assets/twitter.png');
    spriteAlt2 = loadImage('scripts/assets/bird.png');
    spriteAlt3 = loadImage('scripts/assets/obama.png');
    background1 = loadImage('scripts/assets/bkg.png');
    background2 = loadImage('scripts/assets/bkg.png');
    sky1 = loadImage('scripts/assets/clouds.png');
    sky2 = loadImage('scripts/assets/clouds.png');
    sky3 = loadImage('scripts/assets/ships2.png');
    sky4 = loadImage('scripts/assets/ships2.png');
    //music
    music = new SoundFile(this, 'scripts/assets/music.wav');
    music.amp(0.3);
    flap = new SoundFile(this, 'scripts/assets/sfxFlap.wav');
    flap.amp(0.1);
    death = new SoundFile(this, 'scripts/assets/sfxDeath.wav');
    death.amp(1);
    restart(music);
}
...
function setup()
{
    ...
    box = color("#ffc107"); //starting box (for menus) color
    ProBox = color("#ffc107"); //starting color for the promode box so it can flip when clicked
    ProText = color("#ff5722"); //starting color for the promode text so it can flip when clicked
    pipeColor = color("#75ff33"); //starting color for the pipes
    dot = color("#f0fff0");
...
function draw()
{
  ...
  if (mode == 0) //in the menus
  {
    ...
    {
      sans = true;
      sprite = loadImage('scripts/assets/heart.png'); //changes icon to the heart
      background1 = loadImage('scripts/assets/sansBkg.png'); //sets the background images to the sans images
      background2 = loadImage('scripts/assets/sansBkg.png');
      sky1 = loadImage('scripts/assets/sans.png');
  ...
  } else if (mode === 1)
  {
    ...
    for (let i = pipeList.size() - 1; i >= 0; i--) //checks, for every pipe
    {
      let p = pipeList.get(i);
      p.colorTo(pipeColor);
      p.update();
    ...
    if (!isAlive)
    {
      if (sans) //reset stuff if the player was on sans mode
      {
        ...
        {
          ProBox = color("#ffc107");
          ProText = color("#fb8c00");
        } 
        else
        {
          ProBox = color("#fb8c00");
          ProText = color("#ffc107");
        }
        text = color("#FF5722");
        background = color("#4fc3f7");
        box = color("#ffc107");
        pipeColor = color("#75ff33");
        colourPalette = 1;
        pipeSpeed = 3;
        pipeSpace = 100;
  ...
  } else if (mode === 2)
  {
    ...
    //stores highscore
    output = createWriter('scripts/assets/highscore.txt');
  ...
  } else if (mode == 5) //customize screen
  {
    ...
    if (mousePressed) //checks if the player clicks a sprite to change to, back to menu, or slider
    {
      ...
      if (mouseY > boxxY && mouseY < boxxY + 100)
      {
        ...
        {
          sprite = loadImage('scripts/assets/twitter.png');
        }
        if (mouseX > width/2 && mouseX < width/2 + 100)
        {
          sprite = loadImage('scripts/assets/bird.png');
        }
        if (mouseX > width/2 + 200 && mouseX < width/2 + 200 + 100)
        {
          sprite = loadImage('scripts/assets/obama.png');
        }
      }
      ...
      if (mouseY > boxxY + 130 && mouseY < boxxY + 175)
      {
        if (mouseX > width/2 + 200 && mouseX < width/2 + 410 && !justClicked)
        {
          ...
          if (colourPalette % 3 === 2)
          {
            background1 = loadImage('scripts/assets/bkg2.png');
            background2 = loadImage('scripts/assets/bkg2.png');
            sky1 = loadImage('scripts/assets/ships1.png');
            sky2 = loadImage('scripts/assets/ships1.png');
            if (ProBox == box)
            {
              ProBox = color("#3a21c9");
              ProText = color("#8d0104");
            } else
            {
              ProBox = color("#8d0104");
              ProText = color("#3a21c9");
            }
            text = color("#8d0104");
            background = color("#28009f");
            box = color("#3a21c9");
            pipeColor = color("#ff0000");
            dot = color("#F0FFF0");
          }
          if (colourPalette % 3 === 0)
          {
            ...
            {
              ProBox = color("#ffffff");
              ProText = color("#808080");
            }
            text = color("#ffffff");
            background = color("#ffffff");
            box = color("#808080");
            pipeColor = color("#000000");
            dot = color("#4fc3f7");
          }
        }
      }
...
class Pipe
{
  ...
  */
  ...
  Pipe()
  {
      ...
      this.yBottom;
      this.yLenBottom = height;
      this.xLen = 20;
      this.yLen = this.y = height;
      this.c = color("#75FF33");
      this.score = 0;
      this.passed = false;
  }
  ...
  update() //change where the pipe is
  {
  ...
  colorTo(col) //receives color to use
  {
    c = col;
  }
  hits(b) //check if it hits the bird, return if it does
  {
